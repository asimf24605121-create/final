...
-   **Security**: Employs `SameSite`, `HttpOnly`, and `Secure` cookie flags, CORS configuration, session management with device-type enforcement, inactivity timeouts, and session validation. HTTP security headers (`X-Content-Type-Options`, `X-Frame-Options`, `X-XSS-Protection`, `Referrer-Policy`) are set globally. IP resolution trusts `X-Forwarded-For` only from configured trusted proxies (`TRUSTED_PROXIES` env var). Password minimum length is 8 characters. Password changes invalidate all other active sessions. SVG uploads are blocked to prevent XSS. Admin seed password is randomized per install (configurable via `DEFAULT_ADMIN_PASSWORD` env var). Dev-mode password reset tokens are logged server-side only, never returned in API responses.
